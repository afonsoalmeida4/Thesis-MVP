...
uint8_t NwkSkey[16] = {0x0d,0xcd,0x63,0x21,0xfb,0x8d,0xbe,0xcd,0x9b,0xdf,0x13,0x22,0x94,0x29,0x2f,0x69};
// Application Session Key (MSB)
uint8_t AppSkey[16] = {0x98,0x70,0xab,0xc7,0x85,0xf7,0x39,0xa4,0xac,0xa6,0x0e,0xf5,0xd9,0xaf,0xfa,0x0c};
// Device Address (MSB)
uint8_t DevAddr[4] = {0x00,0xfc,0x32,0x72};

volatile int state_door = 0;
...
volatile int get_state() {
  if (digitalRead(PIN_DOOR) == HIGH) { //SE A LEITURA DO PINO FOR IGUAL A HIGH, FAZ
        Serial.println("Aberto"); //ACENDE O LED
        state_door = 1;
        //state_flag = false;
      }
      else { //SENÃO, FAZ
        Serial.println("Fechado"); //APAGA O LED
        state_door = 0;
        //state_flag = false;
      }
      ...
      return state_door;
      //send_lora_temp(state_door);

}



void send_lora_temp( int state)
{   
    /*enable sensors */
...
volatile bool state_flag = false;
...
void setup()
{
    ...
    */
    ...
    //send_lora_temp(state);
    
    
    sodaq_wdt_reset();
    attachInterrupt(PIN_DOOR, interrupt, CHANGE);
  ...
  if (sodaq_wdt_flag) {
    ...
    if (state_flag) {
      ...
      if (!lora.begin()) {
        ...
        send_lora_temp(state);
        setup_rtc_debug();
        //Serial.println("Send Data Lora1");
    ...
    if (isClock)
    {
        isClock = false;
        sodaq_wdt_reset();
        rtc.setAlarmMinutes((rtc.getAlarmMinutes() + 10) % 60); //reschedule the alarm for the next 12h
        if(!lora.begin())
          {
            Serial.println("Failed");
            Serial.println("Check your radio");
